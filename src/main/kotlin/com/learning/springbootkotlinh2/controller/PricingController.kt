...
@RestController
class PricingController( private val pricingSer: PricingSer/*private val pricingService: PricingService,private val pricingRepo: PricingRepo*/) {

    @PostMapping("/pricing")
    fun createPricing(@RequestBody pricingDto: PricingDto): ResponseEntity<PricingDto>  {
        try {
            return ResponseEntity(pricingSer.createPricing(pricingDto), HttpStatus.CREATED)
    ...
    @GetMapping("/pricing")
    fun getAllPricing() : ResponseEntity<List<PricingDto>>{
        return ResponseEntity(pricingSer.getAllPricing(),HttpStatus.OK)
    }

    @GetMapping("/pricing/{pricingId}")
    fun getPricingById(@PathVariable pricingId: Long)=
        ResponseEntity(pricingSer.getPricingById(pricingId),HttpStatus.OK)


    @PutMapping("/pricing/{pricingId}")
    fun updatePricingById(@PathVariable("pricingId") pricingId: Long, @RequestBody pricingDto: PricingDto)/*: ResponseEntity<ProductDto>*/{
        ResponseEntity(pricingSer.updatePricingById(pricingDto), HttpStatus.OK)
    }

    @DeleteMapping("/pricing/{pricingId}")
    fun deletePricingById(@PathVariable("pricingId") pricingId: Long): ResponseEntity<Unit> =
        ResponseEntity(pricingSer.deletePricingById(pricingId),HttpStatus.NO_CONTENT)
